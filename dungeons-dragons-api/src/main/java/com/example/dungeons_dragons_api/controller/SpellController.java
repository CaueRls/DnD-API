...
public class SpellController {
    ...
    @Operation(
            ...
            description = "Retorna uma lista paginada com todas as magias cadastradas."
    )
    ...
    @Operation(
            ...
            description = "Retorna os detalhes de uma magia específica."
    )
    ...
    @Operation(
            ...
            description = "Cadastra uma nova magia. O campo 'school' deve ser um dos valores do enum MagicSchool."
    )
    @RequestBody(
            required = true,
            content = @Content(
                    ...
                    examples = {
                            @ExampleObject(
                                    name = "Bola de Fogo",
                                    summary = "Magia ofensiva de nível 3",
                                    value = """
                    {
                      "name": "Bola de Fogo",
                      "level": 3,
                      "description": "Uma rajada de chamas explode em um ponto que você escolher. Cada criatura em uma esfera de 20 pés de raio centrada nesse ponto deve realizar um teste de resistência de Destreza.",
                      "school": "EVOCATION"
                    }
                    """
                            ),
                            @ExampleObject(
                                    ...
                                    summary = "Magia de cura de nível 1",
                                    value = """
                    {
                      "name": "Curar Ferimentos",
                      "level": 1,
                      "description": "Uma criatura que você toque recupera um número de pontos de vida igual a 1d8 + seu modificador de habilidade de conjuração.",
                      "school": "EVOCATION"
                    }
                    """
                            ),
                            @ExampleObject(
                                    name = "Míssil Mágico",
                                    summary = "Magia ofensiva de nível 1",
                                    value = """
                    {
                      "name": "Míssil Mágico",
                      "level": 1,
                      "description": "Você cria três dardos brilhantes de força mágica. Cada dardo atinge uma criatura de sua escolha que você possa ver dentro do alcance.",
                      "school": "EVOCATION"
                    }
                    """
                            )
    ...
    public ResponseEntity<EntityModel<Spell>> createSpell(
            @org.springframework.web.bind.annotation.RequestBody @Valid Spell spell) {
    ...
    @Operation(
            ...
            description = "Atualiza todos os campos de uma magia pelo ID."
    )
    ...
    public ResponseEntity<EntityModel<Spell>> updateSpell(
            ...
            @org.springframework.web.bind.annotation.RequestBody @Valid Spell spellDetails) {
        Spell spell = repository.findById(id)
                .orElseThrow(() -> new RuntimeException("Magia não encontrada com o ID: " + id));
    ...
    @Operation(
            ...
            description = "Pesquisa magias cujo nome contenha o termo informado (sem distinção de maiúsculas/minúsculas)."
    )
